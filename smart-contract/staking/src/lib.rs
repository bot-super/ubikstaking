...
use anchor_spl::token::{self, Token, TokenAccount, Mint, SetAuthority, Transfer};
use spl_token::instruction::AuthorityType;

declare_id!("C6wGW14B9SUBsG1xVENNvtCeyBgBzbfzbaVaLTtCEuwc");
...
pub mod staking{
    ...
    pub fn unstake_nft(
        ...
        let staking_data = &mut ctx.accounts.staking_data;
        let clock = (&ctx.accounts.clock).unix_timestamp as u64;
        let pool_signer_seeds = &[temp_pool.rand.as_ref(),&[temp_pool.bump]];
        let signer = &[&pool_signer_seeds[..]];

        let number = (clock - staking_data.stake_time) / pool.reward_period;
        let amount = pool.reward_amount[staking_data.badge as usize] * (number-staking_data.claim_number);
        ...
        token::set_authority(cpi_ctx, AuthorityType::AccountOwner, Some(staking_data.staker))?;
...
pub const STAKING_DATA_SIZE : usize = 32+32+32+1+32+8+8+1+1+40;
